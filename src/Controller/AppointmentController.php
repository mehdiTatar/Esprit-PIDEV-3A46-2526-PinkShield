...
class AppointmentController extends AbstractController
{
    ...
    private \App\Service\AiPharmacistService $aiPharmacistService;

    public function __construct(SmsNotificationService $smsNotificationService, AiSymptomAnalyzerService $aiSymptomAnalyzer, \App\Service\AiPharmacistService $aiPharmacistService)
    {
    ...
    #[Route('/ai-suggestions', name: 'appointment_ai_suggestions', methods: ['GET'])]
    public function aiSuggestionsJson(AppointmentRepository $appointmentRepository): JsonResponse
    {
        $this->denyAccessUnlessGranted('IS_AUTHENTICATED_FULLY');
        $user = $this->getUser();
        $appointments = [];
        ...
        return $this->json($aiSuggestions);
    }

    #[Route('/test-ai', name: 'appointment_test_ai', methods: ['POST'])]
    public function testAi(Request $request): JsonResponse
    {
        $data = json_decode($request->getContent(), true);
        $note = $data['note'] ?? 'Patient complains of headache and fever';
        
        try {
            $result = $this->aiPharmacistService->suggestProducts($note);
            return $this->json(['success' => true, 'suggestions' => $result]);
        } catch (\Exception $e) {
            return $this->json(['success' => false, 'error' => $e->getMessage()], 500);
        }
    }

    #[Route('/analyze-note', name: 'appointment_analyze_note', methods: ['POST'])]
    public function analyzeNote(Request $request, \App\Service\AiPharmacistService $aiPharmacistService): JsonResponse
    {
        $this->denyAccessUnlessGranted('IS_AUTHENTICATED_FULLY');
        $data = json_decode($request->getContent(), true);
        $note = $data['note'] ?? '';
        if (empty(trim($note))) {
            return $this->json(['success' => false, 'error' => 'No note provided'], 400);
        }
        try {
            $suggestions = $aiPharmacistService->suggestProducts($note);
    ...
    public function generateInvoice(Appointment $appointment): Response
    {
        ...
        if ($appointment->getParapharmacies()->count() > 0) {
    ...
    #[Route('/checkout-product', name: 'appointment_checkout_product', methods: ['POST'])]
    public function checkoutProduct(Request $request): JsonResponse
    {
        ...
        if (empty($productName)) {
            return $this->json(['error' => 'Product name required'], 400);
        }

        // Try to get Stripe key safely with proper error handling
        try {
        ...
        if (!$stripeKey) {
            return $this->json(['error' => 'Stripe payment is not configured. Please contact the administrator.'], 503);
        }
